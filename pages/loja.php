...
session_start();
    ...
    $resultAtk = $db->query("SELECT * FROM habilidade WHERE nivel_ref = '1' AND atk_def = 1");
    $resultDef = $db->query("SELECT * FROM habilidade WHERE nivel_ref = '1' AND atk_def = 0");
        ...
        <div class="tela">
            ...
            <div class="cartas">
                <div class="buttones" style="margin-bottom: -60px;">
                    <button class="jones">Nivel 1</a</button>
                    <button class="jones" onclick="window.location.href='loja_2.php'">Nivel 2</button>
                    <button class="jones" onclick="window.location.href='loja_3.php'">Nivel 3</button>
               ...
               while ($userdata = mysqli_fetch_assoc($resultAtk)){
                    echo "<div class='img2'>
                            ...
                            <button class='botao_img' onclick=\"window.location.href='view/view.php?id=$userdata[ID]&page=1'\">DESCRIÇÃO</button>";
                            $cardId = $userdata['ID'];
                            ...
                            if(userHasCard($userId, $cardId)){
                                ...
                                echo"<button class='botao_img' style='background-color: green;' onclick=\"window.location.href='comprar.php?id=$userdata[ID]&page=1'\">COMPRAR</button> </div>";
                            }
                }
                while ($userdata = mysqli_fetch_assoc($resultDef)){
                    echo "<div class='img2'>
                            ...
                            <i class='bi bi-coin' style='font-size: 1.2rem; color: green; margin-left: 0px;'></i>
                            <span style='gap: 0;'>$userdata[preco_hab]</span> <!-- Exibe o valor da carta ao lado do ícone -->                            <button class='imagemn'><img src='../dashboard/$userdata[figurinha]' alt='' height='350px' width='250px'></a></button>
                            <button class='botao_img' onclick=\"window.location.href='view/view_def.php?id=$userdata[ID]&page=1'\">DESCRIÇÃO</button>";                            $cardId = $userdata['ID'];
                            $cardId = $userdata['ID'];
