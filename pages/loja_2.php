...
session_start();
    ...
    $resultAtk = $db->query("SELECT * FROM habilidade WHERE nivel_ref = '2' AND atk_def = 1");
    $resultDef = $db->query("SELECT * FROM habilidade WHERE nivel_ref = '2' AND atk_def = 0");
...
<body>
    <div class="universal">
        ...
        <div class="tela">
            <h1 class="loja" style="text-align: center; margin-top: 30px; font-weight: 700; font-size: 60px; font-family: Arial, Helvetica, sans-serif; color: white;">VENDINHA DO RICARDO</h1>
            <div class="cartas">
            <div class="buttones" style="margin-bottom: -60px;">
               ...
               while ($userdata = mysqli_fetch_assoc($resultAtk)){
                    echo "<div class='img2'>
                            ...
                            <button class='botao_img' onclick=\"window.location.href='view/view.php?id=$userdata[ID]&page=2'\">DESCRIÇÃO</button>";
                            $cardId = $userdata['ID'];
                            $nivel = 2;
                            ...
                            if(userHasCard($userId, $cardId)){
                                ...
                                echo"<button class='botao_img' style='background-color: green;' onclick=\"window.location.href='comprar.php?id=$userdata[ID]&page=2'\">COMPRAR</button> </div>";
                                }
                            }
                }
                while ($userdata = mysqli_fetch_assoc($resultDef)){
                    echo "<div class='img2'>
                            ...
                            <i class='bi bi-coin' style='font-size: 1.2rem; color: green; margin-left: 0px;'></i>
                            <span style='gap: 0;'>$userdata[preco_hab]</span> <!-- Exibe o valor da carta ao lado do ícone -->                           
                            <button class='botao_img' onclick=\"window.location.href='view/view_def.php?id=$userdata[ID]&page=2'\">DESCRIÇÃO</button>";                            
                            $cardId = $userdata['ID'];
